...
const options = {
  method: 'GET',
  headers: {accept: 'text/plain', authorization: 'Basic ' + Buffer.from(username + ":" + secret).toString('base64')}
};
...
const calcNodeDiskUsage = (node) => {
    if(node?.nodes) {
        const diskUsedForServicesArray = Object.values(node.nodes).map(service => service.diskUsedGBs)
...
const printResults = () => {
    console.log(`num of distinct user that opened app at least once.... userOpenApps: ${Object.keys(userOpenApps).length}`)
...
function jsonToCSV(jsonData) {
    ...
    // Common headers
    const headers = ["event", "datetime", "distinct_id", "user_id", "nice_node_version", "city", "region", "os", "arch", "platform_release", "device_id", "time", "event_data"]
